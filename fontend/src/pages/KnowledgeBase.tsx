  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 overflow-x-hidden">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 md:mb-10 gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-discord-text tracking-tight mb-2">Kho tri thức</h1>
          <p className="text-gray-500 dark:text-discord-text-muted text-sm md:text-[15px]">Tài liệu chính sách được dùng làm nguồn RAG cho HR Helpdesk AI.</p>
        </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-discord-text">{isAdmin ? 'Tài liệu đã lập chỉ mục' : 'Chủ đề đang được hỏi nhiều'}</h2>
      ...
      {isAdmin ? (
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {pins.length === 0 && <p className="text-sm text-gray-500 dark:text-discord-text-muted">Chưa có chủ đề trending. Hãy hỏi chatbot vài câu về cùng chủ đề.</p>}
          {selectedPin && (
            <div className="basis-full mt-3 rounded-xl border border-[#a6f4df] dark:border-discord-accent/30 bg-white dark:bg-discord-sidebar p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4 mb-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-[#048261] dark:text-discord-accent">TrendPin</p>
              ...
              {selectedPin.citations.length > 0 && (
                <div className="mt-4 border-t border-gray-100 dark:border-discord-bg pt-3">
                  ...
                  <div className="space-y-2">
                    {selectedPin.citations.map((citation, index) => (
                      <div key={`${citation.document_id}-${index}`} className="rounded-lg bg-gray-50 dark:bg-discord-bg p-3">
                        <p className="text-sm font-semibold text-gray-800 dark:text-discord-text">{citation.document_title}</p>
                        <p className="text-xs text-gray-500 dark:text-discord-text-muted line-clamp-2">{citation.excerpt}</p>
                      </div>
                    ))}
                  </div>
                </div>
