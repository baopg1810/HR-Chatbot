...
async def test_admin_frontend_flow_tickets_trending_and_documents(client):
    ...
    escalation = await client.post(
        "/api/v1/chat",
        headers=employee_headers,
        json={"message": "Cho toi xem luong cua Nguyen Van B", "session_id": None},
    )
    assert escalation.status_code == 200
    ticket_id = escalation.json()["escalated_ticket_id"]
    assert ticket_id
    ...
    assert trend.json()["created_pins"][0]["title"] == "Nghỉ phép"
