...
async def test_contract_happy_path_response_shapes(client):
    ...
    assert {"id", "topic_key", "title", "summary", "source_query_count", "citations", "created_at"} <= set(candidate)

    approved_pin = await client.post(
        f"/api/v1/admin/trending/candidates/{candidate['id']}/pin",
        headers=admin_headers,
    )
    assert approved_pin.status_code == 200
    pin = approved_pin.json()
    assert {"id", "topic_key", "title", "summary", "source_query_count", "citations", "created_at", "expires_at"} <= set(pin)
