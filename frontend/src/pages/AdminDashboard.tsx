...
export function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const urgentTickets = tickets.filter((ticket) => ticket.priority === 'high' && !isClosedTicket(ticket));
  ...
  const activities = useMemo<DashboardActivity[]>(() => {
    const ticketActivities = tickets.slice(0, 5).map((ticket) => ({
      ...
      tone: isClosedTicket(ticket) ? 'green' as const : ticket.priority === 'high' ? 'amber' as const : 'blue' as const,
    }));
