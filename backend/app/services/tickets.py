...
def map_db_to_pydantic_ticket(db_ticket: DBTicket) -> Ticket:
    ...
    return Ticket(
        id=format_ticket_id(db_ticket.id),
...
def format_ticket_id(ticket_id: uuid.UUID | str) -> str:
    """Return a short, stable public ticket code while keeping UUID as DB key."""
    ticket_uuid = ticket_id if isinstance(ticket_id, uuid.UUID) else uuid.UUID(str(ticket_id))
    return f"{TICKET_PUBLIC_PREFIX}-{ticket_uuid.hex[:8].upper()}"
...
async def update_ticket(
    ...
    if db is not None:
        ticket_uuid = await _resolve_ticket_uuid(ticket_id, db)
        if not ticket_uuid:
            return None
        ...
        async with get_db_context() as session:
            ticket_uuid = await _resolve_ticket_uuid(ticket_id, session)
            if not ticket_uuid:
                return None
...
async def _resolve_ticket_uuid(ticket_id: str, session) -> uuid.UUID | None:
    if not ticket_id:
        return None
    raw = str(ticket_id).strip()
    if raw.upper().startswith(f"{TICKET_PUBLIC_PREFIX}-"):
        public_id = raw.upper()
        result = await session.execute(select(DBTicket.id))
        for db_ticket_id in result.scalars().all():
            if format_ticket_id(db_ticket_id) == public_id:
                return db_ticket_id
        return None
    return safe_parse_uuid(raw)
