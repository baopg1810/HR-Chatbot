...
from app.models.schemas import Document, DocumentCreate, Role
from app.rag.chunking import chunk_document
from app.rag.loaders import load_policy_document
from app.services.llm import Embedding, embed_document_texts
from app.database.session import get_db_context
from app.models.document import Document as DBDocument
from app.models.document_chunk import DocumentChunk as DBDocumentChunk
...
async def create_document_async(payload: DocumentCreate) -> tuple[Document, int, list[str]]:
    document, indexed_count, warnings = create_document(payload)
    await _persist_document_to_database(document)
    return document, indexed_count, warnings
...
async def create_document_from_upload_async(
    *,
    filename: str,
    content: bytes,
    title: str | None = None,
...
async def delete_document_async(document_id: str) -> Document | None:
...
async def _persist_document_to_database(document: Document) -> None:
    try:
        document_uuid = UUID(document.id)
    except ValueError:
        return

    chunks = [chunk for chunk in list_chunks() if chunk.document_id == document.id]
    try:
        async with get_db_context() as db:
            db_document = await db.get(DBDocument, document_uuid)
            if db_document is None:
                ...
                db_document.document_type = db_document.document_type or "hr_policy"

            await db.execute(delete(DBDocumentChunk).where(DBDocumentChunk.document_id == document_uuid))
            for chunk in chunks:
                db.add(
                    DBDocumentChunk(
                        id=chunk.id,
                        ...
                        page=chunk.page,
                        visibility_roles=chunk.visibility_roles,
        ...
        if get_settings().app_env == "production":
            raise
        print(f"Warning: could not persist document metadata to database: {exc}")


async def _delete_document_from_database(document_id: str) -> None:
    try:
    ...
    try:
        ...
        if get_settings().app_env == "production":
            raise
        print(f"Warning: could not delete document metadata from database: {exc}")
