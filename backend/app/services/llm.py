...
def _build_tool_choice_prompt(query: str, conversation_context: str | None = None) -> str:
    def render(history: str) -> str:
        ...
        return (
            ...
            "Do not choose request_ticket_escalation only because a previous assistant message suggested contacting HR. "
            "The latest user message must explicitly ask to create/open/send a ticket/request, or the conversation "
            "history must show an active ticket flow waiting for ticket details. "
            "A question asking another person's leave balance, payroll, insurance, contract, HR metrics, or private "
            "ticket details is not ticket creation; do not route it to request_ticket_escalation unless the latest "
            "message explicitly asks to create/open/send a ticket.\n"
