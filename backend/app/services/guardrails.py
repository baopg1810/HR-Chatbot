...
_IDENTITY_PATTERNS = [
    ...
    r"\bban\s+ten\s+gi\b",
    r"\bten\s+cua\s+ban\s+la\s+gi\b",
    r"\bban\s+co\s+the\s+lam\s+gi\b",
    r"\bbạn\s+có\s+thể\s+làm\s+gì\b",
    r"\bxin\s+chao\b",
    r"\bhello\b",
    r"\bhi\b",
    r"\bhey\b",
    ...
    r"\bchào\s+bạn\b",
    r"\bbot\s+la\s+gi\b",
    r"\btro\s+ly\s+la\s+gi\b",
    r"\bban\s+duoc\s+tao\s+ra\s+de\s+lam\s+gi\b",
    r"\bban\s+co\s+the\s+giup\s+gi\b",
    r"\bbạn\s+có\s+thể\s+giúp\s+gì\b",
]


def is_identity_question(message: str) -> bool:
    """Return True if the message is a greeting or identity question about the bot itself."""
    normalized = _normalize(message.strip())
    if len(normalized.split()) <= 5 and normalized in {
        "ban la ai", "bạn là ai", "hello", "hi", "hey",
        "xin chao", "chào bạn", "chao ban", "bạn có thể làm gì", "ban co the lam gi",
    }:
        return True
    return _matches_any(normalized, _IDENTITY_PATTERNS)
